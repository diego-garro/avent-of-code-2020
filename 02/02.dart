void main() {
  ...
  int firstPuzzleAnswer = 0;
  int secondPuzzleAnswer = 0;
  List<String> splitPassword;
  RegExp letterCount;

  for (String policy in passwords) {
    if (regexp.hasMatch(policy)) {
      RegExpMatch match = regexp.firstMatch(policy);

      first = int.parse(match.namedGroup('first'));
      second = int.parse(match.namedGroup('second'));
      letter = match.namedGroup('letter');
      pass = match.namedGroup('pass');
      splitPassword = pass.split('');

      letterCount = RegExp('$letter');
      count = letterCount.allMatches(pass).length;

      if (count >= first && count < second + 1) {
        firstPuzzleAnswer++;
      }

      if ((splitPassword[first - 1] == letter ||
              splitPassword[second - 1] == letter) &&
          splitPassword[first - 1] != splitPassword[second - 1]) {
        secondPuzzleAnswer++;
      }
    }
  }

  print('First puzzle, valid passwords: $firstPuzzleAnswer');
  print('Second puzzle, valid passwords: $secondPuzzleAnswer');
}

RegExp regexp = RegExp(
    r'^(?<first>\d{1,2})-(?<second>\d{1,2})\s(?<letter>[a-z]):\s(?<pass>[a-z]{4,})$');
